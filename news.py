...
def get_coin_id_from_symbol(symbol):
    """Return CoinGecko coin_id and name for a given symbol, or (None, None) if not found."""
    try:
        url = f"https://api.coingecko.com/api/v3/coins/list?x_cg_pro_api_key={os.getenv('COINGECKO_API_KEY','')}"
...
def get_coin_stats(symbol):
    """Get stats for a crypto asset using CoinGecko and Twelve Data."""
    coin_id, coin_name = get_coin_id_from_symbol(symbol)
    if not coin_id:
        return f"❌ Could not find crypto asset for symbol: {symbol}"
    # CoinGecko price/market
    try:
        url = f"https://api.coingecko.com/api/v3/coins/{coin_id}?localization=false&tickers=false&market_data=true&community_data=false&developer_data=false&sparkline=false&x_cg_pro_api_key={os.getenv('COINGECKO_API_KEY','')}"
        resp = requests.get(url, timeout=10)
        if resp.status_code == 200:
            data = resp.json()
            ...
            tdata = fetch_twelvedata_stats(symbol, 'crypto', return_dict=True)
            tech_str = ''
            if tdata:
                tech_str = f"\nRSI: {tdata.get('rsi','?')} | MA30: {tdata.get('ma30','?')} | 52w: {tdata.get('range','?')}"
            return f"{coin_name} (${symbol.upper()}): ${price:.4f} {change_str}{tech_str}"
    except Exception:
        pass
    return f"❌ Error fetching stats for {symbol}"

def get_coin_stats_ai(symbol):
    """Get stats and AI summary for a crypto asset using DeepSeek."""
    base = get_coin_stats(symbol)
    # Compose prompt for DeepSeek AI
    prompt = (
        f"Coin: {symbol.upper()}\n"
        f"{base}\n"
...
def get_crypto_ai_summary(symbol):
    """Return an AI-generated summary for the given crypto symbol (stub)."""
    # TODO: Integrate with DeepSeek or other AI API
    return f"[AI summary for {symbol.upper()} goes here.]"